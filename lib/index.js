...
exports.generateStrongPassword = exports.check = void 0;
/**
 ...
 */
const countCharacters = (password, regex) => (password.match(regex) || []).length;
/**
...
const check = (password) => {
    ...
    let strength = STRENGTH_LEVELS.VERY_WEAK.text;
 ...
 */
function generateStrongPassword(config = {}) {
    const defaultConfig = {
        length: 9,
        charCounts: {
            upper: 2,
    ...
    if (remainingLength > 0) {
        const allowedSets = Object.entries(CHAR_SETS)
            .filter(([key]) => finalConfig.charCounts[key] > 0)
            .map(([_, value]) => value)
            .join("");
